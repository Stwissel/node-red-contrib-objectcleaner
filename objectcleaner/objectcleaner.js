...
module.exports = function (RED) {
  ...
  function ObjectCleaner(n) {
        ...
        if (template.hasOwnProperty(prop)) {
          ...
          if (node.strictclean && typeof tProp !== typeof cProp) {
            ...
            if (Array.isArray(tProp) && Array.isArray(cProp)) {
              ...
              if (node.strictclean) {
                cleandit = node.arrayCompare(
                  tProp,
                  cProp,
                  logobject,
                  hasBeenCleaned || cleandit
                );
              }
              // Case 3b : we just leave them
            } else {
              // Deep cleaning ob objects
              cleandit = node.deepclean(
                tProp,
                cProp,
                logobject,
                hasBeenCleaned || cleandit
    ...
    this.hasAllProperties = function (
      template,
      candidate,
      logobject,
      addIfMissing
      ...
      for (var prop in template) {
        if (!candidate.hasOwnProperty(prop)) {
          ...
          if (node.strictclean && typeof tProp !== typeof cProp) {
            ...
            var subresult = node.hasAllProperties(
              tProp,
              cProp,
              logobject,
              addIfMissing
    ...
    // Validation on json object and its non-empty content
    this.validateJSONObjAndNotEmpty = function (jsonObj) {
    ...
    this.on("input", function (msg) {
      ...
      try {
        // Parse template from internal or external reference
        // Do sanity check on the template before further processing
        node.template =
          typeof node.template === "string"
            ? JSON.parse(node.template)
            : node.template;

        if (!node.validateJSONObjAndNotEmpty(node.template)) {
          let errmsg = "Properties must be a JSON object and cannot be empty";
          this.status({
            fill: "red",
            shape: "dot",
            text: errmsg,
          ...
          node.error(errmsg);
          return; // Avoid propagate if error
        }
      } catch (err) {
        this.status({
          fill: "red",
          shape: "dot",
          text: "Please ensure the properties are not empty and in proper JSON format.",
        });
      ...
      try {
        ...
        if (typeof candidate === "object") {
          ...
          if (node.notempty && !node.validateJSONObjAndNotEmpty(candidate)) {
            let errMsg = node.field + " was empty";
            this.status({
              fill: "red",
              shape: "dot",
              text: errMsg,
            ...
            var cleanupNeeded = node.deepclean(
              node.template,
              candidate,
              logobject,
              false
            ...
            var goodToGo = node.hasAllProperties(
              node.template,
              candidate,
              logobject,
              node.musthaveallproperties === "1"
            );
        ...
        this.status({
          fill: "red",
          shape: "dot",
          text: node.field + " cleanup failed",
        });
        node.error(err.message);
      }
    });
